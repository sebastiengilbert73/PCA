...
def main():
    logging.info("pca_toy_images.py main()")
    filepaths = [os.path.join(args.ImagesDirectory, f) for f in os.listdir(args.ImagesDirectory) if
                 ...
                 and f.upper().endswith('.PNG')]

    imagesList = [cv2.imread(filepath, cv2.IMREAD_GRAYSCALE) for filepath in filepaths]

    print ("len(imagesList) = {}".format(len(imagesList)))
    imagePCAModel = PCA.ImagePCAModel.GrayscaleModel(imagesList, zeroThreshold=1e-6)
    ...
    testImage = cv2.imread(os.path.join(args.ImagesDirectory, 'image0.png'), cv2.IMREAD_GRAYSCALE)

    imagePCAModel.TruncateModel(3)
